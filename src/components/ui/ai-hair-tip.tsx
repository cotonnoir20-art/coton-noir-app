...
export function AIHairTip({ 
  ...
  const { currentTip, generateTip, isLoading, error } = useHairTips();
  ...
  if (variant === 'compact') {
    return (
      <div className={`flex items-center gap-3 p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-100 ${className}`}>
        ...
        {showRefresh && !isLoading && (
          <Button
            ...
            className="w-8 h-8 flex-shrink-0 hover:bg-white/50"
          >
            <RefreshCw size={14} />
          </Button>
        )}
  ...
  if (variant === 'featured') {
    return (
      <CotonCard className={`p-6 bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200 ${className}`}>
        <div className="flex items-start justify-between mb-4">
          ...
          {showRefresh && (
            <Button
              ...
              onClick={handleRefresh}
              disabled={isLoading}
              className="border-purple-200 hover:bg-white/50"
            >
              {isLoading ? (
  ...
  // Default variant
  return (
    <CotonCard className={`p-4 bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200 ${className}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <h4 className="font-poppins font-semibold text-sm text-coton-black">
              Conseil d'Amara
            </h4>
            {showRefresh && (
              <Button
